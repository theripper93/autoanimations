...
export class DataSanitizer {
    ...
    static async compilePrimary(flagData, menu, handler) {
        ...
        data.path = await buildFile(data.video.dbSection, data.video)
        return data;
    }
    ...
    static async compileSecondary(flagData, handler) {
        ...
        data.sound = this.setSound(sound, addSoundDelay, handler.systemData.overrideRepeat)
        data.path = secondary.enable ? await buildFile("static", data.video) : "";
    ...
    static async compileSource(handler, flagData, primary) {
        ...
        const sourceFile = data.enable ? await buildFile(data.video.dbSection, data.video) : "";
    ...
    static async compileTarget(flagData) {
        ...
        data.path = data.enable ? await buildFile("static", data.video) : "";
        return data;
    }
    ...
    static async compilePreset(flagData) {
        ...
        async function proToTemp () {
            ...
            data.projectile.path = await buildFile( data.projectile.dbSection, data.projectile)
            data.preExplosion.path = await buildFile(data.preExplosion.dbSection, data.preExplosion)
            data.explosion.path = await buildFile(data.explosion.dbSection, data.explosion)
